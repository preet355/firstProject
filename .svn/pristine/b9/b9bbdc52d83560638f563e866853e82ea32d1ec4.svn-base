						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="page-header">
									<h1>
									    View Requisition Form For <b>${requisitionForm[0].uniqueRequisitionNumber}</b>
									</h1>
									<c:set var="var" value="${module}"/>
									<c:choose>
									 <c:when test="${var=='req'}">
									    <c:choose>
									      <c:when test="${requisitionForm[0].status!='back referred' && requisitionForm[0].status!='approved' && requisitionForm[0].status!='Advertised'}">
									      <a style="float:right;margin-top: -25px;margin-right: 10%;" class="btn btn-sm btn-grey" href="javascript:viewRequisitionRemarkForm('${requisitionForm[0].idRequisition}')">Remarks</a>
								          </c:when>
								        </c:choose>
								     </c:when>
								    </c:choose>
										    ...
										    <table id="simple-table" class="table  table-bordered table-hover">
										      <tbody>
												...
												<%-- <tr>
													 ...
													 <td>
													<label class="pos-rel" style="font-weight:600;font-size:12px;">
														Previous Roster Point Filled 
												    </label>
													</td>
													<td>
													 <label class="pos-rel" style="font-weight:0;font-size:12px;">
														${requisitionForm[0].previosuRosterPoint}
												    </label>
													</td>
												...
												<tr>
												  ...
												  <td>
												     <label class="pos-rel" style="font-weight:0;font-size:12px;">
														${requisitionForm[0].experience} years<c:choose>
														<c:when test="${not empty requisitionForm[0].experienceInMonths}">
														  &nbsp  ${requisitionForm[0].experienceInMonths} months
														</c:when>
														</c:choose>
...
<tr>
											     ...
											     <tr>
											       ...
											       <td>
											         <a id="signedPdf"  title="Download pdf" href="javascript:downloadDoc('${requisitionForm[0].idRequisition}','form')">
													${formTypeDocFileName}
													 </a>
											       </td>
											     </tr>
										    ...
										    <c:when test="${var!='req'}">
										       <c:choose>
										       <c:when test="${requisitionForm[0].status!='submitted' && requisitionForm[0].status!='approved' 
										       && requisitionForm[0].status!='re-submitted' && requisitionForm[0].status!='Advertised'}">
										    <table id="simple-table" class="table  table-bordered table-hover">
										       <tr>
										         <td>
...
function viewRequisitionRemarkForm(idRequisition){
	   document.getElementById('idRequisition').value=idRequisition;
	   document.getElementById('requisitionForm').action="viewRequisitionRemarkForm";
...
function validateCount(){
	...
	function secondUpload(idRequisition){	
		...
		var signedPdf=document.getElementById("signedPdf").innerHTML;
		if(signedPdf.trim().length<1){
		  if(fileUpload.value==''){
			  lblError.innerHTML = "Please Upload the signed PDF";
			  return false;
		  }
		  else{
		document.getElementById('idRequisition').value=idRequisition;
